...
def toggle_color(f, date, selected_dates, commit_count):
	colors = {0: '#171B21', 1: '#1F432B', 2: '#2E6B38', 3: '#52A44E', 4: '#6CD064'}
	current_count = selected_dates.get(date, 0)
	if current_count < commit_count:
		selected_dates[date] = commit_count
		f.config(bg=colors[commit_count])
	else:
...
def create_calendar(year, commit_count):
	...
	for i in range(5):
		button = tk.Button(window, text=str(i), command=lambda i=i: update_commit_count(i))
...
year_input = int(input("Kindly enter the year 📅 in which you wish to paint 🖌️ some custom contributions 🟩: "))
commit_count = 0
